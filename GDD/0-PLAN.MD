...
- [ ] **1.3: Establish Data-Driven Foundation**
    - [ ] Create all the static JSON files as defined in `7-DATA.MD` within the `src/data/` directory.
        ...
        - [ ] `xp_table.json`
        - [ ] `books.json`
    - [ ] Populate these files with the initial v1.0 content from `6-CONTENT.MD`.

- [ ] **1.4: Initialize State Management & Services**
    - [ ] Create placeholder files for all Zustand stores in `src/stores/` as per `5-ARCHITECTURE.MD`.
        - e.g., `useCharacterStore.ts`, `useWorldTimeStore.ts`, etc.
    - [ ] Create placeholder files for all core services in `src/services/`.
        - e.g., `SaveLoadService.ts`, `GameManagerService.ts`, etc.
...
- [ ] **2.1: Implement Zustand Stores**
    - [ ] Flesh out each Zustand store with the state and actions defined in `5-ARCHITECTURE.MD`.
    - [ ] Start with `useWorldTimeStore` and `useCharacterStore` as they are fundamental.

- [ ] **2.2: Implement Core Services**
    - [ ] Implement the `GameManagerService` to handle starting a new game (reading from `character_templates.json` and setting initial store states).
    - [ ] Implement the `SaveLoadService` for web (manual JSON download/upload).

- [ ] **2.3: Main Game Loop & Screen Management**
    - [ ] Create a `Game.tsx` component that will be the main container for all in-game screens.
    - [ ] Refactor `App.tsx` to be the top-level component that initializes stores and renders the correct screen based on game state (e.g., `MainMenu`, `CharacterSelection`, `Game`).
    - [ ] Use `useUIStore` to manage which screen or modal is currently active.
